...
#SBATCH --job-name=exp-31-lstm-sorted-row-locks
#SBATCH --account=def-miranska
#SBATCH --output=logs/exp-31-lstm-sorted-row-locks%A_%a.out
#SBATCH --error=logs/exp-31-lstm-sorted-row-locks%A_%a.err
...
declare -a configs_base=(
    # Base configurations
    "--experiment_name exp-31-lstm-sorted-row-locks/char_ --sort_by start_time-dedupe"
)
...
for horizon in "${horizons[@]}"; do
    for table in "${tables[@]}"; do
        for config in "${configs_base[@]}"; do
            configs+=("$config --horizon $horizon --val_split 0.0 --model lstm --remove_system_tables --token_length_seq --data data/fixed/row_sep/$table.csv")
        done
    done
done
