...
#SBATCH --job-name=exp-46-lstm-rounded-qcut
#SBATCH --account=def-miranska
#SBATCH --output=logs/exp-46-lstm-rounded-qcut_%A_%a.out
#SBATCH --error=logs/exp-46-lstm-rounded-qcut_%A_%a.err
...
declare -a configs_base=(
    # Base configurations
    "--experiment_name exp-46-lstm-rounded-qcut-row-locks/char_ --data data/fixed/row_locks.csv --sort_by start_time-dedupe --binning_method qcut --num_bins 10"
    "--experiment_name exp-46-lstm-rounded-qcut-table-locks/char_ --data data/fixed/table_locks.csv --sort_by start_time-dedupe --binning_method qcut --num_bins 10"
)
