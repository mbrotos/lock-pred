...
#SBATCH --job-name=exp-33-tranformer-random
#SBATCH --account=def-miranska
#SBATCH --output=logs/exp-33-tranformer-random_%A_%a.out
#SBATCH --error=logs/exp-33-tranformer-random_%A_%a.err
...
declare -a configs_base=(
    # Base configurations
    "--experiment_name exp-33-tranformer-random-row-locks/char_ --data data/fixed/row_locks.csv --sort_by start_time-dedupe --rounding_bin_size 10000"
    "--experiment_name exp-33-tranformer-random-table-locks/char_ --data data/fixed/table_locks.csv --sort_by start_time-dedupe --rounding_bin_size 10000"
)
