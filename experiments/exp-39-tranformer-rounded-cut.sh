...
declare -a configs_base=(
    # Base configurations
    "--experiment_name exp-39-tranformer-rounded-cut-row-locks_${NUM_BINS}/char_ --data data/fixed/row_locks.csv --sort_by start_time-dedupe --binning_method cut --num_bins ${NUM_BINS}"
    "--experiment_name exp-39-tranformer-rounded-cut-table-locks_${NUM_BINS}/char_ --data data/fixed/table_locks.csv --sort_by start_time-dedupe --binning_method cut --num_bins ${NUM_BINS}"
)
