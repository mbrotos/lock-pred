...
#SBATCH --job-name=exp-23-lstm-row-id-local-sorted-row-locks
#SBATCH --account=def-miranska
#SBATCH --output=logs/exp-23-lstm-row-id-local-sorted-row-locks_%A_%a.out
#SBATCH --error=logs/exp-23-lstm-row-id-local-sorted-row-locks_%A_%a.err
...
declare -a configs_base=(
    # Base configurations
    "--experiment_name exp-23-lstm-row-id-local-sorted-row-locks/char_ --sort_by start_time"
)
...
for horizon in "${horizons[@]}"; do
    for table in "${tables[@]}"; do
        for config in "${configs_base[@]}"; do
            configs+=("$config --horizon $horizon --val_split 0.0 --model lstm --remove_system_tables --token_length_seq --data data/fixed/row_sep/$table.csv --add_row_id --add_label_tokens")
        done
    done
done
