...
#SBATCH --job-name=exp-44-transformer-local-rounded-cut
#SBATCH --account=def-miranska
#SBATCH --output=logs/exp-44-transformer-local-rounded-cut%A_%a.out
#SBATCH --error=logs/exp-44-transformer-local-rounded-cut%A_%a.err
...
declare -a configs_base=(
    # Base configurations
    "--experiment_name exp-44-transformer-local-rounded-cut_${NUM_BINS}/char_ --sort_by start_time-dedupe --binning_method cut --num_bins ${NUM_BINS}"
)
...
for horizon in "${horizons[@]}"; do
    for table in "${tables[@]}"; do
        for config in "${configs_base[@]}"; do
            configs+=("$config --horizon $horizon --val_split 0.2 --checkpoint --model transformer --remove_system_tables --token_length_seq --data data/fixed/row_sep/$table.csv")
        done
    done
done
