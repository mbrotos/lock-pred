...
#SBATCH --job-name=exp-35-naive-rounded
#SBATCH --account=def-miranska
#SBATCH --output=logs/exp-35-naive-rounded_%A_%a.out
#SBATCH --error=logs/exp-35-naive-rounded_%A_%a.err
...
declare -a configs_base=(
    # Base configurations
    "--experiment_name exp-35-naive-rounded-row-locks/char_ --data data/fixed/row_locks.csv --sort_by start_time-dedupe --naive_baseline --rounding_bin_size 10000"
    "--experiment_name exp-35-naive-rounded-table-locks/char_ --data data/fixed/table_locks.csv --sort_by start_time-dedupe --naive_baseline --rounding_bin_size 10000"
)
