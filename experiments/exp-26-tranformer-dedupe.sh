...
#SBATCH --job-name=exp-26-tranformer-dedupe
#SBATCH --account=def-miranska
#SBATCH --output=logs/exp-26-tranformer-dedupe_%A_%a.out
#SBATCH --error=logs/exp-26-tranformer-dedupe_%A_%a.err
...
declare -a configs_base=(
    # Base configurations
    "--experiment_name exp-26-tranformer-dedupe-row-locks/char_ --data data/fixed/row_locks.csv --sort_by start_time-dedupe"
    "--experiment_name exp-26-tranformer-dedupe-table-locks/char_ --data data/fixed/table_locks.csv --sort_by start_time-dedupe"
)
