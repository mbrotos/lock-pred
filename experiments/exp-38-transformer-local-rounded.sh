...
#SBATCH --job-name=exp-38-transformer-local-rounded
#SBATCH --account=def-miranska
#SBATCH --output=logs/exp-38-transformer-local-rounded%A_%a.out
#SBATCH --error=logs/exp-38-transformer-local-rounded%A_%a.err
...
declare -a configs_base=(
    # Base configurations
    "--experiment_name exp-38-transformer-local-rounded/char_ --sort_by start_time-dedupe --rounding_bin_size 10000"
)
