...
#SBATCH --job-name=exp-6-longer
#SBATCH --account=def-miranska
#SBATCH --output=logs/exp-6-longer_%A_%a.out
#SBATCH --error=logs/exp-6-longer_%A_%a.err
...
declare -a configs_base=(
    # Base configurations
    "--experiment_name exp-6-longer/char_ --data data/table_locks.csv"
    "--experiment_name exp-6-longer/char_ --data data/table_locks_large.csv"
)
...
for horizon in "${horizons[@]}"; do
    for config in "${configs_base[@]}"; do
        configs+=("$config --horizon $horizon --val_split 0.0 --model transformer --remove_system_tables --token_length_seq --seq_length 100")
    done
done
