...
#SBATCH --job-name=exp-50-transformer-local-rounded-qcut
#SBATCH --account=def-miranska
#SBATCH --output=logs/exp-50-transformer-local-rounded-qcut%A_%a.out
#SBATCH --error=logs/exp-50-transformer-local-rounded-qcut%A_%a.err
...
declare -a configs_base=(
    # Base configurations
    "--experiment_name exp-50-transformer-local-rounded-qcut/char_ --sort_by start_time-dedupe --binning_method qcut --num_bins 10"
)
