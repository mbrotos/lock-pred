...
#SBATCH --job-name=exp-19-transformer-sorted-row-locks
#SBATCH --account=def-miranska
#SBATCH --output=logs/exp-19-transformer-sorted-row-locks_%A_%a.out
#SBATCH --error=logs/exp-19-transformer-sorted-row-locks_%A_%a.err
...
declare -a configs_base=(
    # Base configurations
    "--experiment_name exp-19-transformer-sorted-row-locks/char_ --sort_by start_time"
)
