...
#SBATCH --job-name=exp-36-naive-local-rounded-row-locks
#SBATCH --account=def-miranska
#SBATCH --output=logs/exp-36-naive-local-rounded-row-locks_%A_%a.out
#SBATCH --error=logs/exp-36-naive-local-rounded-row-locks_%A_%a.err
...
declare -a configs_base=(
    # Base configurations
    "--experiment_name exp-36-naive-local-rounded-row-locks/char_ --sort_by start_time-dedupe --naive_baseline  --rounding_bin_size 10000"
)
