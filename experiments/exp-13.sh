...
declare -a configs_base=(
    # Base configurations
    "--experiment_name exp-6-row_id/char_r_lr_ --add_row_id --add_label_tokens --data data/fixed/row_locks.csv"
...
for horizon in "${horizons[@]}"; do
    for config in "${configs_base[@]}"; do
        configs+=("$config --horizon $horizon --val_split 0.0 --model transformer --remove_system_tables --token_length_seq --results_dir results/fixed")
    done
done
