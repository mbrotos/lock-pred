...
#SBATCH --job-name=exp-11-row-locks-sep-naive
#SBATCH --account=def-miranska
#SBATCH --output=logs/exp-11-row-locks-sep-naive_%A_%a.out
#SBATCH --error=logs/exp-11-row-locks-sep-naive_%A_%a.err
...
declare -a configs_base=(
    # Base configurations
    "--experiment_name fixed/row_sep/exp-11-row-locks-naive/char_"
)
...
for horizon in "${horizons[@]}"; do
    for table in "${tables[@]}"; do
        for config in "${configs_base[@]}"; do
            configs+=("$config --horizon $horizon --val_split 0.0 --model transformer --remove_system_tables --token_length_seq --data data/fixed/row_sep/$table.csv --naive_baseline")
        done
    done
done
