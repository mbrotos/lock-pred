...
#SBATCH --job-name=exp-48-naive-local-rounded-qcut-row-locks
#SBATCH --account=def-miranska
#SBATCH --output=logs/exp-48-naive-local-rounded-qcut-row-locks_%A_%a.out
#SBATCH --error=logs/exp-48-naive-local-rounded-qcut-row-locks_%A_%a.err
...
declare -a configs_base=(
    # Base configurations
    "--experiment_name exp-48-naive-local-rounded-qcut-row-locks/char_ --sort_by start_time-dedupe --naive_baseline  --binning_method qcut --num_bins 10"
)
