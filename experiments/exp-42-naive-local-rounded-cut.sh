...
#SBATCH --job-name=exp-42-naive-local-rounded-cut-row-locks
#SBATCH --account=def-miranska
#SBATCH --output=logs/exp-42-naive-local-rounded-cut-row-locks_%A_%a.out
#SBATCH --error=logs/exp-42-naive-local-rounded-cut-row-locks_%A_%a.err
...
declare -a configs_base=(
    # Base configurations
    "--experiment_name exp-42-naive-local-rounded-cut-row-locks_${NUM_BINS}/char_ --sort_by start_time-dedupe --naive_baseline  --binning_method cut --num_bins ${NUM_BINS} --disable_cache"
)
...
for horizon in "${horizons[@]}"; do
    for table in "${tables[@]}"; do
        for config in "${configs_base[@]}"; do
            configs+=("$config --horizon $horizon --val_split 0.0 --model transformer --remove_system_tables --token_length_seq --data data/fixed/row_sep/$table.csv")
        done
    done
done
